...
void readfile(char *filename, yORn echototerm, etudiant *list, yORn import){
    ...
    if(file != NULL){
        while(fgets(line, MAX_SIZE, file) != NULL){
            ...
            if((!(entryls % 2)) && entryls != 0 && import){
                ...
                , &entryls //Dirty fix
                , &new_etudiant->etud_info.apogee, &new_etudiant->etud_info.nom, &new_etudiant->etud_info.prenom, &new_etudiant->etud_info.genre, &new_etudiant->etud_info.filiere
                , &new_etudiant->etud_info.date_inscription.jour, &new_etudiant->etud_info.date_inscription.mois, &new_etudiant->etud_info.date_inscription.annee
                , &new_etudiant->etud_info.graduation_date.jour, &new_etudiant->etud_info.graduation_date.mois, &new_etudiant->etud_info.graduation_date.annee
                ...
                for(int i = 0; i<7; i++){
                    if(!(&new_etudiant->etud_info.modules[i].module_name)){
